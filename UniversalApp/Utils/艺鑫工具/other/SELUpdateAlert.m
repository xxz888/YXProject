 ...
 */
+ (void)showUpdateAlertWithVersion:(NSString *)version Descriptions:(NSArray *)descriptions focTag:(BOOL)tagBOOl
{
    ...
    SELUpdateAlert *updateAlert = [[SELUpdateAlert alloc]initVersion:version description:description focTag:tagBOOl];
...
- (instancetype)initVersion:(NSString *)version description:(NSString *)description focTag:(BOOL)tagBOOl
{
    self = [super init];
    if (self) {
        self.version = version;
        self.desc = description;
        self.tagBOOl = tagBOOl;
        [self _setupUI];
    }
    return self;
}
...
- (void)_setupUI
{
    ...
    self.cancelButton.center = CGPointMake(CGRectGetMaxX(updateView.frame), CGRectGetMinY(updateView.frame));
    self.cancelButton.bounds = CGRectMake(0, 0, Ratio(36), Ratio(36));
    [self.cancelButton setImage:[[UIImage imageNamed:@"VersionUpdate_Cancel"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] forState:UIControlStateNormal];
