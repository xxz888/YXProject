...
#define itemY   Height/1.4-30  //最上面item的y坐标
#define distance 400
...
-(instancetype)initWithButtonBGImageArr:(NSArray *)imgArr andButtonBGT:(NSArray *)title titlsTag:(NSArray *)titlesTag{
    self = [super initWithFrame:CGRectMake(0, 0, Width, Height)];
    if (self) {
        images = [NSArray arrayWithArray:imgArr];
        titles = [NSArray arrayWithArray:title];
        titlesTagArray  = [NSArray arrayWithArray:titlesTag];
        angle = 0;
        [self loadSubViews];
    }
    return self;
...
-(void)loadSubViews {
    ...
    bgImageView.image = [UIImage imageNamed:@"背景1"];
...
//    [_backGroundView addSubview:bgImageView];
    ...
    for (int i = 0; i<2; i++) {
        for (int j = 0; j<images.count; j++) {
            ...
            item.frame = CGRectMake(marginX +(Row+itemW)*(j%3), itemY+(itemH+marginY)*(j/3)+distance, itemW, itemH);
            ...
            UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, itemW+9, itemW, 21)];
            label.text = titles[j];
            ...
            UILabel *labelTag = [[UILabel alloc] initWithFrame:CGRectMake(0, itemW+9 + 28, itemW+10, 21)];
            labelTag.text = titlesTagArray[j];
            labelTag.font = [UIFont systemFontOfSize:14];
            labelTag.textAlignment = NSTextAlignmentCenter;
            labelTag.textColor = [UIColor darkGrayColor];
            [item addSubview:labelTag];
    ...
    _bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, Height-49, Width, 49)];
...
//    [_backGroundView addSubview:_bottomView];
    ...
    _exitImgvi.frame = CGRectMake(Width/2-29/2, 10, 29, 29);
