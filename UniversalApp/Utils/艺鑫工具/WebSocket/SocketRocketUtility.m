...
- (void)webSocket:(SRWebSocket *)webSocket didCloseWithCode:(NSInteger)code reason:(NSString *)reason wasClean:(BOOL)wasClean {
    if (webSocket == self.socket) {
        NSLog(@"************************** socket连接断开************************** ");
        NSLog(@"被关闭连接，被关闭连接，code:%ld,reason:%@,wasClean:%d",(long)code,reason,wasClean);
        [QMUITips showInfo:[NSString stringWithFormat:@"socket 连接失败 code:%ld,reason:%@,wasClean:%d",(long)code,reason,wasClean]];;
